 ...
 * @date 2020/8/25
 */
public class XyDsUserMapperExtXmlFactory implements IMapperXmlFactory {

    private IMapperFactory mapperFactory;

    public XyDsUserMapperExtXmlFactory(IMapperFactory mapperFactory) {
    ...
    public MapperXml build(String tableName) {
        ...
        JavaConfig cfg = ConfigTools.getConfig("/themes/xy/dsuser/config/mapper-ext-xml.yaml", JavaConfig.class);
        BeanUtils.copyProperties(cfg, xml);
        MapperClass mapperClass = mapperFactory.build(tableName);
        xml.setMapperClass(mapperClass);

        String fileName = StrUtil.format("{}ext/{}.xml", cfg.getFilePath(), mapperClass.getClassName());
        fileName = fileName.replaceFirst("DAO", "ExtMapper");
